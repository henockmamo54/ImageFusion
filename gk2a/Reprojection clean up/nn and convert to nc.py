...
grid_x_, grid_y_ = np.mgrid[ 
                            min(latitude_):max(latitude_) + 1e-10: ((max(latitude_)-min(latitude_))/latitude.shape[0]),
                            min(longitude_):max(longitude_) + 1e-10: ((max(longitude_)-min(longitude_))/longitude.shape[1])
                            ]
