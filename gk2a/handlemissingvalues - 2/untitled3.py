# -*- coding: utf-8 -*-
"""
Created on Tue Jul 13 13:50:21 2021
...
path="D:/Workplace/githubProjects/ImageFusion/GK2A/test2"
...
enddate = datetime.datetime.strptime("12-31-2019", '%m-%d-%Y')
datediff=abs((enddate - base).days)

date_list = [(base + datetime.timedelta(days=x)).strftime("%Y%m%d") for x in range(datediff)]
...
def create_file_from_source( trg_file):
    ...
    for name, var in src.variables.items():
        ...
        if(name=="value"): 
            image_pixel_values = np.empty(src.variables['value'][:].shape) 
...
print("generate null values for missing values")
