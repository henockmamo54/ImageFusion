...
from GK2A_angle import calc_solar_angle, calc_sat_angle, ncdump
...
for year in years:
    for jday in jdays:
        for hour in hours:
            ...
            Args = np.vstack([lats,lons]).T
            a = parfun(Fun, Args, Cores=100, check=True)
            os.chdir('/bess21/Sungchan/GK2A_RAD/INPUT/SZA_BC')
            # savemat('CRK2020_NIR_ANGLE.mat', mdict={'ANGLE': a})
            savemat('GK2A_ANGLE_KOREA2020_%03d.mat' % (n+4958), mdict={'SZA': a})

# Fun = lambda x,y: np.array(calc_sat_angle(x,y, 0, 128.2))
# Args = np.vstack([lats,lons]).T
# b = parfun(Fun, Args, Cores=100, check=True)
