# -*- coding: utf-8 -*-
"""
Created on Fri Jul  2 09:14:22 2021
...
python3 _cropping.py 38.22346787684907 127.21710138948873 38.18195837298332 127.27222505323994
...
left_upper_lat= float(sys.argv[1])+(2.0 *np.sign(float(sys.argv[1])))
left_upper_lon= float(sys.argv[2])-(2.0 *np.sign(float(sys.argv[2])))
right_lower_lat=float(sys.argv[3])-(2.0 *np.sign(float(sys.argv[3])))
right_lower_lon=float(sys.argv[4])+(2.0 *np.sign(float(sys.argv[4])))

if(abs(left_upper_lat)>=90):
    left_upper_lat= 90 * np.sign(left_upper_lat)
    
if(abs(right_lower_lat)>=90):
    right_lower_lat= 90 * np.sign(right_lower_lat )
    
if(abs(left_upper_lon)>=180):
    left_upper_lon= 180 * np.sign(left_upper_lon )
    
if(abs(right_lower_lon)>=180):
    right_lower_lon= 180 * np.sign(right_lower_lon )
...
path ="/bess19/Image_fusion/download/GK2A/L1/VI006/2019/"  
pathoutput="/bess19/Image_fusion/pre_process/GK2A/Cropped/VI006/" 
CT_path='/bess19/Image_fusion/code/download_cropping_reprojection/GK2A/conversion_table/'

if not os.path.exists(pathoutput):
    os.makedirs("/bess19/Image_fusion/pre_process/GK2A/Cropped/VI006/")
...
for root, dirs, files in os.walk(path):
    ...
    for file in files:
        try:
            input_ncfile_path = (path+file)        
            output_ncfile_path=(pathoutput+file)   
            ...
            ############################
            # input_ncfile_path = 'gk2a_ami_le1b_vi008_fd010ge_201912310300.nc'
            ...
            (ulc_lin,ulc_col)=lincol_from_latlon_geos(0.5,left_upper_lat,left_upper_lon)
            (lrc_lin,lrc_col)=lincol_from_latlon_geos(0.5,right_lower_lat,right_lower_lon)
