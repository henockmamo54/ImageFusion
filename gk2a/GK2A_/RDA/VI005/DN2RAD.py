...
if not os.path.exists("/bess19/Image_fusion/pre_process/GK2A/RAD/VI005/"):
     os.mkdir("/bess19/Image_fusion/pre_process/GK2A/RAD/VI005/")

inpath2 = '/bess19/Image_fusion/download/GK2A/L1/VI005/2019/'
input_ncfile_paths = [inpath2 + '/' + x for x in os.listdir(inpath2) if 'vi005' in x]
output_npy_paths = [x[-43:-3] + '.npy' for x in input_ncfile_paths if 'vi005' in x]
output_txt_paths = [x[-43:-3] + '.txt' for x in input_ncfile_paths if 'vi005' in x]
...
for input_ncfile_path, output_txt_path, output_npy_path in zip(input_ncfile_paths, output_txt_paths, output_npy_paths):
    try:
        ...
        os.chdir('/bess19/Image_fusion/pre_process/GK2A/RAD/VI005')
