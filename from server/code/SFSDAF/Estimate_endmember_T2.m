...
function endmember_delta = Estimate_endmember_T2(fraction_T1,CR_T2,CR_T1,xL,yL,class,bands)
...
endmember_delta = zeros(bands,class);
for b=1:bands  
%%%%%%%%%%%%%%%%%%%%%%%
...
lb = ones(1,class) * min(min(CR_T2(:,:,b)));
ub = ones(1,class) * max(max(CR_T2(:,:,b)));
  
options = optimoptions('fmincon','Display','off');
[x,fval] = fmincon(@(x) objfun_deltaEndm(x,fraction_T1,CR_T2(:,:,b),thresholdMatrix,xL,yL),x0,[],[],[],[],lb,ub,[],options);
